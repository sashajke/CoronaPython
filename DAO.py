...
def orm(cursor, dto_type):
    ...
    return [row_map(col_mapping, dto_type) for row in cursor.fetchall()]
...
class Dao:
    ...
    def update(self, set_values, cond):
        ...
        # stmt = "UPDATE clinics SET demand=demand-40 WHERE location='Tel_Aviv'"
        self._conn.execute(stmt)

    def getLastInsertedId(self):
        cursor = self._conn.cursor()
        cursor.execute('SELECT id FROM ', self._table_name, ' ORDER BY id DESC LIMIT 1')
